...
ALTER TABLE orders ADD CONSTRAINT orders_payment_method_check CHECK (payment_method IN ('Vorauszahlung','Bei Lieferung','Guthaben','PayPal'));
...
UPDATE legal_pages SET body='Hier müssen insbesondere Verantwortlicher, Zwecke und Rechtsgrundlagen der Verarbeitung, Empfänger/Auftragsverarbeiter (z. B. Render, Neon, Cloudinary, Resend), Speicherdauer, Betroffenenrechte, Cookies/Sessions und Kontaktangaben vollständig beschrieben werden. Diese Startvorlage ersetzt keine Rechtsberatung.' WHERE slug='datenschutz' AND body LIKE 'Bitte trage hier deine Datenschutzerklärung%';
...
-- Phase 4: PayPal-Zahlung mit manueller Owner-Bestätigung
ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_payment_method_check;
ALTER TABLE orders ADD CONSTRAINT orders_payment_method_check CHECK (payment_method IN ('Vorauszahlung','Bei Lieferung','Guthaben','PayPal'));
...
SELECT setval('order_number_seq', GREATEST(COALESCE((SELECT MAX(order_number) FROM orders),999),999), true);
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS paypal_paid_confirmed_at TIMESTAMPTZ;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS paypal_paid_confirmed_by UUID REFERENCES users(id) ON DELETE SET NULL;

-- Phase 5: Redesign, sichere Ablehnungen und interner Staff-Chat
ALTER TABLE orders ADD COLUMN IF NOT EXISTS denial_reason TEXT NOT NULL DEFAULT '';
...
CREATE INDEX IF NOT EXISTS idx_staff_chat_messages_created_at ON staff_chat_messages(created_at DESC);
ALTER TABLE orders ADD COLUMN IF NOT EXISTS refund_required BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS refund_confirmed_at TIMESTAMPTZ;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS refund_confirmed_by UUID REFERENCES users(id) ON DELETE SET NULL;
