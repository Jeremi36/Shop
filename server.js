...
function parseSupplierAvailability(html) {
  ...
  const qty = qtyMatch ? safeStockInt(qtyMatch[1], null) : null;
  const counterQty = stockCounter ? safeStockInt(stockCounter[1], null) : null;
  const detectedStock = counterQty !== null ? counterQty : qty;

  if (/schema\.org\/outofstock/i.test(raw) || /itemprop=["']availability["'][^>]*content=["']out[_-]?of[_-]?stock["']/i.test(raw) || /class=["'][^"']*outOfStock[^"']*selected/i.test(raw) || /stockcounter[^>]*notonstock[^>]*>\s*0x\s*</i.test(raw) || /nicht\s+auf\s+lager/i.test(raw) || detectedStock === 0) {
    return { status: 'out_of_stock', note: 'Beim Händler leer', stock: 0 };
  }
  if (/schema\.org\/instock/i.test(raw) || /itemprop=["']availability["'][^>]*content=["']in[_-]?stock["']/i.test(raw) || /(^|\b)auf\s+lager(\b|$)/i.test(raw) || (detectedStock !== null && detectedStock > 0)) {
    return { status: 'in_stock', note: detectedStock !== null ? `Beim Händler verfügbar (${detectedStock}x)` : 'Beim Händler verfügbar', stock: detectedStock };
  }
