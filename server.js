...
app.get('/', async (req, res) => {
  ...
  const products = categoryId
    ? await pool.query(`SELECT p.*, c.name AS category_name FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.active=true AND p.category_id=$1 ORDER BY p.created_at DESC`, [categoryId])
    : await pool.query(`SELECT p.*, c.name AS category_name FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.active=true ORDER BY p.created_at DESC`);
  res.render('shop', { title: 'Shop', products: products.rows, categories: categories.rows, selectedCategory: categoryId });
});
...
app.get('/staff/products', requireStaff, async (req, res) => {
  ...
  res.render('staff-products', { title: 'Produkte verwalten', products: products.rows, categories: categories.rows });
});
