...
app.post('/staff/products', requireStaff, upload.single('image'), async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const product = await client.query('INSERT INTO products (name,description,price_cents,stock,discount_percent,image_url,category_id) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id', [name, description, cents, parseInt(stock,10), parseInt(discount_percent || '0',10), imageUrl, category_id || null]);
...
app.post('/staff/products/:id', requireStaff, upload.single('image'), async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    await client.query('UPDATE products SET name=$1, description=$2, price_cents=$3, stock=$4, discount_percent=$5, active=$6, image_url=$7, category_id=$8 WHERE id=$9', [name, description, cents, parseInt(stock,10), parseInt(discount_percent || '0',10), active === 'on', imageUrl, category_id || null, req.params.id]);
