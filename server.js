...
function parseSupplierAvailability(html) {
  ...
  // Wichtig: Mr-Joy/Lightspeed enthält oft beide Texte im HTML:
  // "Nicht auf Lager" UND "Auf Lager". Einer davon ist per class="hidden" versteckt.
  // Deshalb darf man NICHT einfach global nach "Nicht auf Lager" suchen.
  ...
  const availabilityMatch = firstMatch(raw, [
    ...
    /"availability"\s*:\s*"([^"]+)"/i,
    /schema\.org\/(InStock|OutOfStock)/i
  ]);
  const availability = availabilityMatch ? String(availabilityMatch[1]).toLowerCase() : '';

  // Selected variant ist stärker als generische Texte auf der Seite.
  // Beispiel in-stock:
  // class="select-variant select-variant2 selected" ... <span class="stockcounter">359x</span>
  // Beispiel out-of-stock:
  // class="select-variant select-variant2 outOfStock selected" ... <span class="stockcounter notonstock">0x</span>
  const selectedVariant = raw.match(/<label\b[^>]*class=["']([^"']*select-variant[^"']*selected[^"']*)["'][\s\S]*?<\/label>/i);
  let selectedQty = null;
  let selectedOut = false;
  if (selectedVariant) {
    const selectedBlock = selectedVariant[0];
    const selectedClass = selectedVariant[1] || '';
    selectedOut = /(^|\s)outOfStock(\s|$)/i.test(selectedClass) || /stockcounter[^>]*notonstock[^>]*>\s*0x\s*</i.test(selectedBlock);
    const selectedCounter = selectedBlock.match(/stockcounter[^>]*>\s*(\d+)x\s*</i);
    if (selectedCounter) selectedQty = safeStockInt(selectedCounter[1], null);
  }

  const anyCounter = raw.match(/stockcounter(?![^>]*notonstock)[^>]*>\s*(\d+)x\s*</i);
  const counterQty = selectedQty !== null ? selectedQty : (anyCounter ? safeStockInt(anyCounter[1], null) : null);
  const detectedStock = counterQty !== null ? counterQty : metaQty;

  const visibleOut = /<span\b[^>]*class=["'](?!(?=[^"']*(?:^|\s)hidden(?:\s|$)))[^"']*out-of-stock[^"']*["'][^>]*>\s*Nicht\s+auf\s+Lager\s*<\/span>/i.test(raw);
  const visibleIn = /<(?:strong|span)\b[^>]*class=["'](?!(?=[^"']*(?:^|\s)hidden(?:\s|$)))[^"']*in-stock[^"']*["'][^>]*>\s*Auf\s+Lager\s*<\/(?:strong|span)>/i.test(raw);

  if (selectedOut || /out[_-]?of[_-]?stock/i.test(availability) || /schema\.org\/outofstock/i.test(raw) || visibleOut || detectedStock === 0) {
    return { status: 'out_of_stock', note: 'Nicht auf Lager', stock: 0 };
  }

  if (/in[_-]?stock/i.test(availability) || /schema\.org\/instock/i.test(raw) || visibleIn || (detectedStock !== null && detectedStock > 0)) {
    return { status: 'in_stock', note: detectedStock !== null ? `Lager: ${detectedStock}` : 'Auf Lager', stock: detectedStock };
  }

  return { status: 'unknown', note: 'Konnte Lagerstatus nicht sicher erkennen', stock: null };
}
...
async function checkSupplierStatus(product, { force=false } = {}) {
  ...
  } else if (result.status === 'in_stock' && result.stock !== null && result.stock !== undefined) {
    const stock = safeStockInt(result.stock, 0);
    const variants = await pool.query('SELECT id FROM product_variants WHERE product_id=$1 AND active=true ORDER BY name', [product.id]);
...
app.post('/staff/products/profit-category', requireStaff, async (req, res) => {
  const profit = Math.max(0, Number(req.body.target_profit_percent) || 0);
  const categoryId = req.body.category_id || '';
  if (!categoryId) return res.redirect('/staff/products');
  // Verkaufspreis = Einkaufspreis + Profit %. Nur Produkte mit Einkaufspreis > 0 werden angepasst.
  await pool.query(`
    UPDATE products
    SET target_profit_percent=$1,
        price_cents=GREATEST(0, ROUND(purchase_price_cents * (1 + ($1::numeric / 100.0)))::int)
    WHERE category_id=$2 AND COALESCE(purchase_price_cents,0) > 0
  `, [profit, categoryId]);
