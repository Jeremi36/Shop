...
const googleDriveRequiredKeys = [
  'GOOGLE_DRIVE_FOLDER_ID',
  'GOOGLE_CLIENT_ID',
  'GOOGLE_CLIENT_SECRET',
  'GOOGLE_REFRESH_TOKEN'
];
const googleDriveMissingKeys = googleDriveRequiredKeys.filter((key) => !process.env[key]);
const googleDriveConfigured = googleDriveMissingKeys.length === 0;
...
async function storeProductImage(file, productId) {
  ...
  if (googleDriveConfigured) {
    ...
    console.log(`[Google Drive] Produktbild gespeichert: ${fileName} (${fileId})`);
    return `/drive-image/${fileId}`;
  }

  if (!localImageFallbackEnabled) {
    throw new Error(`Google Drive ist nicht vollständig konfiguriert. Fehlende Render-Variablen: ${googleDriveMissingKeys.join(', ')}. Deshalb wurde das Bild NICHT lokal gespeichert, weil lokale Render-Uploads bei Redeploys verschwinden würden.`);
  }
...
app.get('/drive-image/:fileId', async (req, res) => {
  try {
    const drive = getDriveClient();
    if (!drive) return res.status(503).send('Google Drive ist nicht konfiguriert.');
    ...
    media.data.on('error', (err) => {
      console.error('[Google Drive] Bild-Stream Fehler:', err.message);
      if (!res.headersSent) res.status(500).end('Bild konnte nicht geladen werden.');
      else res.end();
    });
    media.data.pipe(res);
  } catch (err) {
    console.error('[Google Drive] Bild konnte nicht geladen werden:', err.message);
...
app.listen(port, () => {
  ...
  if (googleDriveConfigured) {
    console.log('[Google Drive] OAuth-Bildspeicher ist aktiv. Produktbilder werden in Google Drive gespeichert.');
  } else {
    console.warn(`[Google Drive] Nicht aktiv. Fehlende Variablen: ${googleDriveMissingKeys.join(', ')}. Uploads werden blockiert, außer LOCAL_IMAGE_FALLBACK=true ist gesetzt.`);
  }
});
