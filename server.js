...
const googleDriveConfigured = !!(process.env.GOOGLE_DRIVE_FOLDER_ID && process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL && process.env.GOOGLE_PRIVATE_KEY);
...
app.post('/staff/products', requireStaff, upload.single('image'), async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const product = await client.query('INSERT INTO products (name,description,price_cents,purchase_price_cents,target_profit_percent,stock,discount_percent,image_url,category_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id', [name, description, cents, purchaseCents, parseInt(target_profit_percent || '0',10), parseInt(stock,10), parseInt(discount_percent || '0',10), null, category_id || null]);
