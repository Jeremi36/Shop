...
app.post('/orders/place', requireLogin, requireCustomer, async (req, res) => {
  ...
  try {
    ...
    const order = await client.query(`INSERT INTO orders (user_id,assigned_staff_id,address_snapshot,total_cents,payment_method,payment_status,delivery_status) VALUES ($1,$2,$3,$4,$5,$6,'Not Started') RETURNING *`, [res.locals.user.id, assignedStaffId, address, total, paymentMethod, paymentStatus]);
    for (const item of cart.rows) {
      await client.query(`INSERT INTO order_items (order_id,product_id,product_name,variant_name,variety,unit_price_cents,purchase_price_cents,discount_percent,quantity) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [order.rows[0].id, item.id, item.name, item.variant_name || null, item.variant_name || '', item.price_cents, item.purchase_price_cents || 0, item.discount_percent, item.quantity]);
...
app.get('/staff', requireStaff, async (req, res) => {
  ...
  const stats = await pool.query(`SELECT
    (SELECT COUNT(*)::int FROM orders WHERE archived_by_staff=false ${filter}) orders,
...
app.post('/staff/products', requireStaff, upload.single('image'), async (req, res) => {
  const { name, description, price, stock, discount_percent, category_id, purchase_price, target_profit_percent } = req.body;
  ...
  try {
    await client.query('BEGIN');
    const product = await client.query('INSERT INTO products (name,description,price_cents,purchase_price_cents,target_profit_percent,stock,discount_percent,image_url,category_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id', [name, description, cents, purchaseCents, parseInt(target_profit_percent || '0',10), parseInt(stock,10), parseInt(discount_percent || '0',10), imageUrl, category_id || null]);
    await replaceProductVariants(client, product.rows[0].id, parseVariants(req.body.variants));
    await client.query('COMMIT');
...
app.post('/staff/products/:id', requireStaff, upload.single('image'), async (req, res) => {
  const { name, description, price, stock, discount_percent, active, category_id, purchase_price, target_profit_percent } = req.body;
  const cents = priceToCents(price);
  const purchaseCents = priceToCents(purchase_price);
  ...
  try {
    await client.query('BEGIN');
    await client.query('UPDATE products SET name=$1, description=$2, price_cents=$3, purchase_price_cents=$4, target_profit_percent=$5, stock=$6, discount_percent=$7, active=$8, image_url=$9, category_id=$10 WHERE id=$11', [name, description, cents, purchaseCents, parseInt(target_profit_percent || '0',10), parseInt(stock,10), parseInt(discount_percent || '0',10), active === 'on', imageUrl, category_id || null, req.params.id]);
...
app.get('/staff/orders', requireStaff, async (req, res) => {
  ...
  const whereByFilter = {
    ...
    Unpaid: "o.payment_status='Unpaid'",
    'Delivery in Progress': "o.delivery_status='Delivery in Progress'",
  ...
  const { rows } = await pool.query(`
    SELECT o.*, u.email, u.full_name, s.full_name AS assigned_staff_name
    FROM orders o
...
app.post('/staff/orders/:id/update', requireStaff, async (req, res) => {
  ...
  res.redirect('/staff/orders?filter=' + encodeURIComponent(delivery_status === 'Delivered' ? 'Delivered' : status));
});
