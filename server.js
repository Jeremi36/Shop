...
app.post('/staff/products', requireStaff, upload.single('image'), async (req, res) => {
  const { name, description, price, stock, discount_percent, category_id, purchase_price } = req.body;
  ...
  try {
    await client.query('BEGIN');
    const product = await client.query('INSERT INTO products (name,description,price_cents,purchase_price_cents,target_profit_percent,stock,discount_percent,image_url,category_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id', [name, description, cents, purchaseCents, targetProfit, parseInt(stock,10), parseInt(discount_percent || '0',10), null, category_id || null]);
...
app.post('/staff/products/:id', requireStaff, upload.single('image'), async (req, res) => {
  const { name, description, price, stock, discount_percent, active, category_id, purchase_price } = req.body;
  const cents = priceToCents(price);
  const purchaseCents = priceToCents(purchase_price);
  const targetProfit = profitPercentFromPrices(cents, purchaseCents);
  const client = await pool.connect();
  try {
    ...
    await client.query('UPDATE products SET name=$1, description=$2, price_cents=$3, purchase_price_cents=$4, target_profit_percent=$5, stock=$6, discount_percent=$7, active=$8, image_url=$9, category_id=$10 WHERE id=$11', [name, description, cents, purchaseCents, targetProfit, parseInt(stock,10), parseInt(discount_percent || '0',10), active === 'on', imageUrl, category_id || null, req.params.id]);
